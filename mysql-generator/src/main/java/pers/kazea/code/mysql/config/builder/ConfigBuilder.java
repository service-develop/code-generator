package pers.kazea.code.mysql.config.builder;

import org.apache.commons.lang3.StringUtils;
import pers.kazea.code.mysql.DbType;
import pers.kazea.code.mysql.InjectionConfig;
...
import pers.kazea.code.mysql.config.*;
import pers.kazea.code.mysql.config.po.TableField;
import pers.kazea.code.mysql.config.po.TableInfo;
import pers.kazea.code.mysql.config.rules.NamingStrategy;
...
import static pers.kazea.generator.common.utils.ClassHelper.getSuperClassName;
 ...
 * @since 2016-08-30
 */
public class ConfigBuilder implements TemplateConfig {

    /**
     * 模板路径配置信息
     */
    private final DbTemplateConfig template;
    /**
     * 数据库配置
     */
    ...
    public ConfigBuilder(PackageConfig packageConfig, DataSourceConfig dataSourceConfig, StrategyConfig strategyConfig,
                         DbTemplateConfig template, GlobalConfig globalConfig) {
        // 全局配置
        ...
        // 模板配置
        if (null == template) {
            this.template = new DbTemplateConfig();
        } else {
    ...
    @Override
    public Map<FileType, String> getTemplatePath() {
        return null;
    }
     ...
     * @return 所以模板路径配置信息
     */
    public DbTemplateConfig getTemplate() {
        return template == null ? new DbTemplateConfig() : template;
    }
     ...
     * </p>
     *
     * @param template  DbTemplateConfig
     * @param outputDir
     * @param config    PackageConfig
     */
    private void handlerPackage(DbTemplateConfig template, String outputDir, PackageConfig config) {
        // 包信息
    ...
    private boolean tableNameMatches(String setTableName, String dbTableName) {
        return setTableName.equals(dbTableName) || matches(setTableName, dbTableName);
    }
    ...
    @Override
    public List<Map<String, Object>> getObjectMaps() {
        List<Map<String, Object>> list = new ArrayList<>();
        this.tableInfoList.forEach(t -> list.add(getObjectMap(t)));
        return list;
    }
    ...
    public Map<String, Object> getObjectMap(TableInfo tableInfo) {
        ...
        if (this.getStrategyConfig().isControllerMappingHyphenStyle()) {
            objectMap.put("controllerMappingHyphenStyle", this.getStrategyConfig().isControllerMappingHyphenStyle());
//            objectMap.put("controllerMappingHyphen", StringUtils.camelToHyphen(tableInfo.getEntityPath()));
        }
        objectMap.put("restControllerStyle", this.getStrategyConfig().isRestControllerStyle());
        objectMap.put("config", this);
        objectMap.put("package", this.getPackageInfo());
        ...
        objectMap.put("entityBuilderModel", this.getStrategyConfig().isEntityBuilderModel());
        ...
        objectMap.put("superControllerClassPackage", this.getSuperControllerClass());
